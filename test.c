...
typedef struct m4test_s {
    ...
    m4test_stack dbefore, rbefore, dafter, rafter;
} m4test;
...
= {
   ...
   {"__literal0",
    { (m4int)m4__literal0, (m4int)m4bye, },
    { 0 },           { 0 },
    { 1, {0} },      { 0 },
   },
   {"literal",
...
static m4int m4test_run(m4th* m, const m4test* t) {
    ...
    m4test_stack_copy(&t->rbefore, &m->rstack);
    m4th_enter(m);
    return m4test_stack_equal(&t->dafter, &m->dstack)
        && m4test_stack_equal(&t->rafter, &m->rstack);
}
...
static void m4test_failed(m4th* m, const m4test* t, FILE* out) {
    ...
    m4test_stack_print(&t->rafter, out);
