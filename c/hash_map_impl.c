...
H(m4hash_map_) * H(m4hash_map_new_)(H(m4u) capacity) {
    ...
    map = (H(m4hash_map_) *)m4mem_allocate(sizeof(H(m4hash_map_)) +
                                           capacity * sizeof(H(m4hash_map_entry_)));
    map->size = 0;
    map->lcap = lcap;
    map->vec = (H(m4hash_map_entry_) *)(map + 1);
    for (i = 0; i < capacity; i++) {
        H(m4hash_map_entry_) *e = map->vec + i;
        e->key = 0;
        e->val = 0;
...
static inline m4cell H(m4hash_map_entry_present_)(const H(m4hash_map_entry_) * e) {
    return e->next != H(m4hash_no_entry_);
}

static inline m4cell H(m4hash_map_entry_next_)(const H(m4hash_map_entry_) * e) {
    return e->next != H(m4hash_no_next_);
}

static inline H(m4u) H(m4hash_map_indexof_)(const H(m4hash_map_) * map, H(m4) key) {
...
static const H(m4hash_map_entry_) *
    H(m4hash_map_find_in_collision_list_)(const H(m4hash_map_) * map, H(m4) key,
                                          const H(m4hash_map_entry_) * bucket) {
    while (key != bucket->key) {
        if (!H(m4hash_map_entry_next_)(bucket)) {
            return NULL;
        }
...
const H(m4hash_map_entry_) * H(m4hash_map_find_)(const H(m4hash_map_) * map, H(m4) key) {
    if (map->size != 0) {
        ...
        if (H(m4hash_map_entry_present_)(e)) {
...
static H(m4u) H(m4hash_pick_collision_index_)(H(m4hash_map_) * map, H(m4u) pos) {
    ...
    for (H(m4u) i = 0; i < cap; i++) {
        ...
        const H(m4hash_map_entry_) *entry = map->vec + collision_pos;
        if (!H(m4hash_map_entry_present_)(entry)) {
...
static void H(m4hash_store_)(H(m4hash_map_) * map, H(m4hash_map_entry_) * to, H(m4) key, m4cell val,
                             H(m4u) next) {
    to->key = key;
    to->val = val;
    to->next = next;
...
const H(m4hash_map_entry_) * H(m4hash_map_insert_)(H(m4hash_map_) * map, H(m4) key, m4cell val) {
    ...
    if (pos != H(m4hash_no_entry_)) {
        H(m4hash_map_entry_) *collision_entry = map->vec + pos;
