...
static m6cell m6token_print_consumed_ip(m6token tok, const m6token *code, m6cell maxn,
    ...
    const m6cell nbytes = m6token_consumes_ip(tok);
...
static m6cell m6_2bytes_copy_to_token(uint16_t src, m6token *dst) {
    memcpy(dst, &src, sizeof(src));
    return sizeof(src) / SZt;
}
static m6cell m6_4bytes_copy_to_token(uint32_t src, m6token *dst) {
    memcpy(dst, &src, sizeof(src));
    return sizeof(src) / SZt;
}
static m6cell m6_8bytes_copy_to_token(uint64_t src, m6token *dst) {
    memcpy(dst, &src, sizeof(src));
    return sizeof(src) / SZt;
}
...
void m6slice_copy_to_code(const m6slice src, m6code *dst) {
    ...
    while (i < sn && j < dn) {
        ...
        if (x == m6_lit_string_) {
            ...
            delta = m6_2bytes_copy_to_token(len, ddata + j);
            i += delta, j += delta;
        ...
        case M6FLAG_CONSUMES_IP_2:
            ...
            delta = m6_2bytes_copy_to_token((uint16_t)sdata[i], ddata + j);
            i += delta, j += delta;
            break;
        case M6FLAG_CONSUMES_IP_4:
            ...
            delta = m6_4bytes_copy_to_token((uint32_t)sdata[i], ddata + j);
            i += delta, j += delta;
            break;
        case M6FLAG_CONSUMES_IP_8:
            ...
            delta = m6_8bytes_copy_to_token((uint64_t)sdata[i], ddata + j);
            i += delta, j += delta;
            break;
        }
    }
